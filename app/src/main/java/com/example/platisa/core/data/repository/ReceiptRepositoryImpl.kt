...
import com.example.platisa.core.data.helper.BillDuplicateDetector
import com.example.platisa.core.data.helper.DuplicateCheckResult
import com.example.platisa.core.data.mapper.toDomain
import com.example.platisa.core.data.mapper.toEntity
...
class ReceiptRepositoryImpl @Inject constructor(
    ...
    override suspend fun insertReceipt(receipt: Receipt, billingPeriod: String?): Long { // Added param
        ...
        if (receipt.category == com.example.platisa.core.domain.model.BillCategory.RESTAURANT) {
    ...
    override fun getEpsAnalyticsData(): Flow<List<com.example.platisa.core.domain.model.EpsMonthData>> {
        return epsDao.getEpsAnalyticsData().map { entities ->
            try {
                ...
                entities.filter { it.date != null }
                    ...
                    .map { (month, entries) ->
                        com.example.platisa.core.domain.model.EpsMonthData(
                            month = month,
    ...
    override suspend fun insertReceiptItems(items: List<com.example.platisa.core.domain.model.ReceiptItem>, receiptId: Long) {
        val entities = items.map { item ->
            com.example.platisa.core.data.database.entity.ReceiptItemEntity(
                receiptId = receiptId,
                name = item.name,
    ...
    override suspend fun getReceiptItems(receiptId: Long): List<com.example.platisa.core.domain.model.ReceiptItem> {
        return receiptDao.getReceiptItems(receiptId).map { entity ->
            com.example.platisa.core.domain.model.ReceiptItem(
                name = entity.name,
    ...
    override suspend fun deleteDuplicateReceipts(): Int {
        ...
        fun scoreReceipt(receipt: com.example.platisa.core.data.database.entity.ReceiptEntity): Int {
            var score = 0
    ...
    override suspend fun insertReceipts(receipts: List<Receipt>): List<Long> {
        ...
        val preparedEntities = mutableListOf<com.example.platisa.core.data.database.entity.ReceiptEntity>()
        var skippedCount = 0
