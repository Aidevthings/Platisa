...
class ReviewReceiptViewModel @Inject constructor(
    ...
    fun processManualUrl(url: String) {
        ...
        if (com.platisa.app.core.common.FiscalScraper.isFiscalUrl(trimmedUrl)) {
            android.util.Log.d("ReviewVM", "Processing Manual URL: $trimmedUrl")
            launchCatching {
                 val scrapeResult = com.platisa.app.core.common.FiscalScraper.scrapeFiscalData(trimmedUrl)
                 when (scrapeResult) {
    ...
    private fun processImage(targetUri: Uri) {
        ...
        launchCatching {
            ...
            if (false && existingReceipt != null) {
                ...
                val qrCodeData = passedQrData ?: com.platisa.app.core.common.QrCodeExtractor.extractQrCode(targetUriString, context)
                
                // DEBUG: Log result
                val qrStatusIdx = if (qrCodeData != null) "SUCCESS (${qrCodeData.take(15)}...)" else "FAILED"
                _rawText.value = "🔍 QR SCAN STATUS: $qrStatusIdx\n" +
                        "🔑 GEMINI KEY CHECK: ${if (com.platisa.app.BuildConfig.GEMINI_API_KEY.isNotBlank()) "✅ FOUND" else "❌ MISSING"}\n"

                var fiscalParsed: ParsedReceipt? = null
                
                // 2. If it is a fiscal URL, try scraping it
                if (qrCodeData != null && com.platisa.app.core.common.FiscalScraper.isFiscalUrl(qrCodeData)) {
                    android.util.Log.d("ReviewVM", "Detected Fiscal QR URL. Attempting to scrape...")
                    val scrapeResult = com.platisa.app.core.common.FiscalScraper.scrapeFiscalData(qrCodeData)
